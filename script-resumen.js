...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function calcularImporteTotal() {
    const costoFelino = 8000;
    const costoCanino = 10000;
  ...
  const mercadoPagoQrDiv = document.getElementById('mercado-pago-qr');
  const transferenciaBancariaQrDiv = document.getElementById('transferencia-bancaria-qr');

  wayPayRadios.forEach(radio => {
    radio.addEventListener('change', function() {
      mercadoPagoQrDiv.style.display = radio.value === 'mercado-pago' && radio.checked ? 'block' : 'none';
      transferenciaBancariaQrDiv.style.display = radio.value === 'transferencia-bancaria' && radio.checked ? 'block' : 'none';
    });
  });

  const radioSeleccionado = Array.from(wayPayRadios).find(radio => radio.checked);
  mercadoPagoQrDiv.style.display = radioSeleccionado && radioSeleccionado.value === 'mercado-pago' ? 'block' : 'none';
  transferenciaBancariaQrDiv.style.display = radioSeleccionado && radioSeleccionado.value === 'transferencia-bancaria' ? 'block' : 'none';
...
form.addEventListener('submit', function (e) {
  ...
  const message = `¡Hola! Quiero confirmar mi turno con los siguientes detalles:\n\n` +
    ...
    `¿Mascota preñada?: ${tableData.preñada}\n` +
